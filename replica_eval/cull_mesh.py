...
parser.add_argument('--input_mesh', type=str,  help='path to the mesh to be culled', default='../data/Replica/cull_GTmesh/office0.ply')
parser.add_argument('--input_scalemat', type=str,  help='path to the scale mat')
parser.add_argument('--traj', type=str,  help='path to the trajectory', default='../data/Replica/scan1/traj.txt')
parser.add_argument('--output_mesh', type=str,  help='path to the output mesh', default='../data/Replica/cull_GTmesh/office0')
...
cy = 339.5
scale = 6553.5
# os.makedirs(args.output_mesh, exist_ok=True)

for idx in range(1):
    output_mesh_file = args.output_mesh
    # output_mesh_file = args.output_mesh + '/{}_10.ply'.format(idx)
    poses = load_poses(args.traj)
    # gap = len(poses) // 10
    # poses = poses[gap*idx:gap*(idx+1)]
