...
class SceneDatasetDN(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        if gt_depth:
            depth_paths = glob_data(os.path.join('{0}'.format(self.instance_dir), 'depth', "*.npy"))
        else:
        ...
        if incremental != 0:
            # TODO: ignore the last part images
            step_num = len(image_paths) // incremental
            if cross:
                tmp = [0, 4, 5, 6, 3, 7, 1, 9, 8, 2]
                step_i = tmp[step_i]
                self.camera_idxs = [*range(step_num*step_i, step_num*(step_i+1))]
            else:
                if accumulate_data:
                    self.camera_idxs = [*range(0, step_num*(step_i+1))]
                else:
                    self.camera_idxs = [*range(step_num*step_i, step_num*(step_i+1))]
            self.n_batches = len(self.camera_idxs)
            self.camera_idxs.extend(keyf_camera_idxs)
        elif block != '':
            _step_i = int(block.split('/')[0])
            _incremental = int(block.split('/')[1])
            _step_num = len(image_paths) // _incremental
            self.camera_idxs = [*range(_step_num*_step_i, _step_num*(_step_i+1))]
            self.n_batches = len(self.camera_idxs)
            self.camera_idxs.extend(keyf_camera_idxs)
        else:
            self.camera_idxs = [*range(len(image_paths))]
            self.n_batches = len(self.camera_idxs)

        print('Used camera idxs are: ', self.camera_idxs)
        self.n_images = len(self.camera_idxs)

        self.cam_file = '{0}/cameras.npz'.format(self.instance_dir)
        camera_dict = np.load(self.cam_file)
        scale_mats = [camera_dict['scale_mat_%d' % idx].astype(np.float32) for idx in self.camera_idxs]
        world_mats = [camera_dict['world_mat_%d' % idx].astype(np.float32) for idx in self.camera_idxs]
        ...
        self.rgb_images = []
        for path in np.array(image_paths)[self.camera_idxs].tolist():
            rgb = rend_util.load_rgb(path)
            rgb = rgb.reshape(3, -1).transpose(1, 0)
            self.rgb_images.append(torch.from_numpy(rgb).float())
        ...
        self.depth_images = []
        self.normal_images = []
        for dpath, npath in zip(np.array(depth_paths)[self.camera_idxs].tolist(),
                                np.array(normal_paths)[self.camera_idxs].tolist()):
            depth = np.load(dpath)
        ...
        if mask_paths is None:
            ...
            for path in np.array(mask_paths)[self.camera_idxs].tolist():
                mask = np.load(path)
