...
def plot(implicit_network, indices, plot_data, path, epoch, plot_3d_freq, img_res, plot_nimgs, resolution, grid_boundary,  level=0):
    ...
    if plot_data is not None:
        ...
        plot_uncertainty_maps(plot_data['uncertainty_map'], plot_data['rgb_eval'], path, epoch, plot_nimgs, img_res, indices)
...
def plot_uncertainty_maps(uncertainty_maps, rgb_maps, path, epoch, plot_nrow, img_res, indices):
    uncertainty_maps_plot = lin2img(uncertainty_maps, img_res)
    uncertainty_maps *= 1e13
    uncertainty_maps = uncertainty_maps.clamp(0, 1)
    ...
    tensor = torchvision.utils.make_grid(uncertainty_maps_plot,
                                         ...
                                         nrow=plot_nrow).cpu().detach().numpy()
    tensor = tensor.transpose(1, 2, 0)
    print('max: {}, min: {}'.format(np.max(tensor), np.min(tensor)))

    save_path = '{0}/uncertainty_{1}_{2}.png'.format(path, epoch, indices[0])
    
    plt.imsave(save_path, tensor[:, :, 0], cmap='seismic')
