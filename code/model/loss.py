...
loss_beta = lambda loss, b : torch.mean(loss / (2 * (b+0.1) ** 2))
beta_reg = lambda b : torch.mean(torch.log(b+0.1)) + 3
...
class MonoSDFLoss(nn.Module):
    def __init__(self, rgb_loss, 
        ...
        if self.use_uncertainty_loss != 0:
            self.rgb_loss = utils.get_class(rgb_loss)(reduction='none')
        else:
    ...
    def get_uncertainty_loss(self, loss, uncertainty):
        dis_loss = loss_beta(loss, uncertainty)
        beta_reg_loss = beta_reg(uncertainty) / 2
        return dis_loss, beta_reg_loss
    ...
    def get_depth_loss(self, depth_pred, depth_gt, mask):
        ...
        lenth = int(np.sqrt(depth_pred.shape[0]))
        return self.depth_loss(depth_pred.reshape(1, lenth, lenth), (depth_gt * 50 + 0.5).reshape(1, lenth, lenth), mask.reshape(1, lenth, lenth))
    ...
    def forward(self, model_outputs, ground_truth):
        ...
        if self.gt_depth:
            d_mask = (depth_gt != 0) & mask
            depth_loss = F.l1_loss(depth_pred[None][d_mask], depth_gt[d_mask])
        else:
        ...
        # rgb_loss:        [1024, 3]
        if self.use_uncertainty_loss == 0:
            uncert_l = torch.tensor(0.0).cuda().float()
            reg_l = torch.tensor(0.0).cuda().float()
        elif self.use_uncertainty_loss == 1:
            pixel_l = rgb_loss.mean(1)
            uncert_l, reg_l = self.get_uncertainty_loss(pixel_l, model_outputs['uncertain_C_values'].reshape(-1))
        ...
        uncertainty_loss = self.uncertainty_weight * uncert_l + reg_l
        loss += uncertainty_loss

        output = {
            'loss': loss,
            'rgb_loss': rgb_loss,
            'uncert_l': uncert_l * self.uncertainty_weight,
