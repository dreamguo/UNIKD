...
class ImplicitNetwork(nn.Module):
    ...
    def get_pot_feature(self, input):
        if self.embed_fn is not None:
            input = self.embed_fn(input)
...
class RenderingNetwork(nn.Module):
    def __init__(
        ...
        if final_act == 'sigmoid':
            ...
            self.final_act = torch.nn.Softplus()
    ...
    def forward(self, points, normals, view_dirs, feature_vectors, indices):
        ...
        for l in range(0, self.num_layers - 1):
            ...
            if l >= self.num_layers - 4 and l < self.num_layers - 2:
                pot_feature.append(x)
...
class MonoSDFNetwork(nn.Module):
    def __init__(self, conf):
        ...
        self.uncertain_C_network = RenderingNetwork(self.feature_vector_size, **conf.get_config('uncertain_C_network'))
    ...
    def forward(self, input, indices=None):
        ...
        if self.runtime_info:
            points_flat = points_flat[:1]
            dirs_flat = dirs_flat[:1]
            from thop import profile
            flops, params = profile(self.implicit_network, inputs=(points_flat, ))
            print('FLOPs = ' + str(flops/1000**1) + 'K')
            print('Params = ' + str(params/1000**2) + 'M')
        sdf, feature_vectors, gradients = self.implicit_network.get_outputs(points_flat)
        pod_feature_d = self.implicit_network.get_pot_feature(points_flat)
        if self.runtime_info:
            flops, params = profile(self.rendering_network, inputs=(points_flat, gradients, dirs_flat, feature_vectors, indices, ))
            print('FLOPs = ' + str(flops/1000**1) + 'K')
            print('Params = ' + str(params/1000**2) + 'M')
        rgb_flat, pod_feature_c = self.rendering_network(points_flat, gradients, dirs_flat, feature_vectors, indices)
        rgb = rgb_flat.reshape(-1, N_samples, 3)
        if self.runtime_info:
            flops, params = profile(self.uncertain_C_network, inputs=(points_flat, gradients, dirs_flat, feature_vectors, indices, ))
            print('FLOPs = ' + str(flops/1000**1) + 'K')
            print('Params = ' + str(params/1000**2) + 'M')
            ipdb.set_trace()
        # uncertainty modeling for color
        uncertain_C_flat, _ = self.uncertain_C_network(points_flat, gradients, dirs_flat, feature_vectors, indices)
        uncertain_C = uncertain_C_flat.reshape(-1, N_samples, 1)

        weights = self.volume_rendering(z_vals, sdf)

        rgb_values = torch.sum(weights.unsqueeze(-1) * rgb, 1)
        uncertain_C_values = torch.sum(weights.unsqueeze(-1) * uncertain_C, 1)
        ...
        output = {
            ...
            'uncertain_C_values': uncertain_C_values,
            'depth_values': depth_values,
            'z_vals': z_vals,
            'depth_vals': z_vals * depth_scale,
            'sdf': sdf.reshape(z_vals.shape),
            'weights': weights,
            'pod_feature_d': pod_feature_d,
            'pod_feature_c': pod_feature_c,
        }
