...
def work(args):
    ...
    gap = len(poses) // 10
    poses = poses[(args.idx*gap) : ((args.idx+1)*gap)]
    ...
    if 0:
        ...
        y_poses[0, 1, 3] = 1
        y_poses[1, 1, 3] = 2
        vis_poses = torch.stack(poses).numpy()
        vis_poses[:, :3, 3] *= 10
        visualize_poses([vis_poses[::5], x_poses, y_poses])
